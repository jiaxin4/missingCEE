...
source("DA_functions_binary_outcome.R")
library(rootSolve) 
library(mgcv)
library(tidyverse)
expit <- function(x){
...
for (i_miss in 1:length(params_list)){
  ...
  if (i_miss == 1){
    ## define the binary outcome: dichotomize by mean
    ...
    dat_new <- dat %>% 
      ...
      mutate(logsteps30 = log(steps30 + 0.5),
             ...
             steps30_binary_median = ifelse(steps30 > median(steps30, na.rm = TRUE), 1, 0))
  }

  ## comparison (1) complete case
  dat_complete_case <- dat_new[which(!is.na(dat_new$steps30_binary_median)),]
  ...
  missingness_model_formula <- as.formula("steps30_observe_indicator ~ s(decision.index, by = send)  + homework.location*send + is.weekday*send + steps30pre.zero.bin_56*send")
  ...
  result <- loglink_cee_dr(dat = dat_new, 
  ...
  ##################
  emee_fit <- emee(data = dat_complete_case,
                   id = "user",
                   outcome = "steps30_binary_median",
                   treatment = "send",
                   rand_prob = 0.6,
  ...
  hold <- summary(emee_fit)$causal_excursion_effect
  cc_beta_colored <- cbind(hold[,1], hold[,2], hold[,3])
  
  #################
  emee_fit <- emee(data = dat_imputezero,
                   id = "user",
                   outcome = "steps30_binary_median",
                   treatment = "send",
                   rand_prob = 0.6,
  ...
  #################
  emee_fit <- emee(data = dat_imputemean,
                   id = "user",
                   outcome = "steps30_binary_median",
                   treatment = "send",
                   rand_prob = 0.6,
  ...
  saveRDS(dat_marginal, "loglink_addmissingness_marginal.RDS")
  
  # 2.1 moderated effect: time -----------------------------------------------
  moderator <- "decision.index" #c() or "send"
  # id <- "user"
  # treatment <- "send"
  # outcome <- "steps30_binary_median"
  # response_observe_indicator <- "steps30_observe_indicator"
  ...
  saveRDS(dat_moderated_time, "loglink_addmissingness_moderated_time.RDS")
  
  
  # 2.3 moderated effect: weekday -----------------------------------------------
  moderator <- "is.weekday" #c() or "send"
  # id <- "user"
  # treatment <- "send"
  # outcome <- "steps30_binary_median"
  # response_observe_indicator <- "steps30_observe_indicator"
  ...
  saveRDS(dat_moderated_weekday, "loglink_addmissingness_moderated_weekday.RDS")
